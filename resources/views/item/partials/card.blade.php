<div class="rounded bg-white border p-4">
    ...
    <div class="grid grid-cols-2 gap-2 mt-4">
        <button class="viewProductBtn w-full px-4 py-1 bg-zinc-600 text-white border border-transparent rounded-md font-semibold text-xs text-white tracking-widest hover:bg-zinc-800 active:bg-gray-200 active:text-black focus:outline-none focus:ring-2 focus:ring-green-500 focus:ring-offset-2 transition ease-in-out duration-150" data-id="{{ $item->id }}" data-name="{{ $item->name }}" data-description="{{ $item->description }}" data-price="{{ $item->price }}" data-image="{{ asset('img/products/' . $item->image) }}">
            View
        </button>
